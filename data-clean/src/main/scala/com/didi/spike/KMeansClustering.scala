...
object KMeansClustering {
  def main(args: Array[String]) {
    if (args.length < 5) {
      println("Usage:KMeansClustering trainingDataFilePath testDataFilePath numClustersnumIterations runTimes")
      sys.exit(1)
    }
    ...
    val rawTrainingData = sc.textFile(this.getClass.getResource("Wholesale customers data.csv").toString)
